...
root = Tk()
root.title("Калькулятор в действии")
...
def division_click():
    ...
    if operation_number == 0:
        ...
        try:
            ...
            e.insert(0, 'Деление на 0 исключено')
...
def positive_negative():
    try:
        ...
        print("Нет числа для применения")
        pass
...
def delete():
    try:
        ...
        print("Деление на 0 исключено")
        pass

def add_point():
    current_number = e.get()
    if "." in current_number:
        print("Число уже имеет точку")
        pass
    else:
        if current_number != "":
...
button_clear = Button(root, borderwidth=2, text="CE",fg='red', padx=37, pady=20,command= lambda: clear_entry()).grid(row=4, column=0)

button_minus = Button(root, borderwidth=2, text="-", padx=38, pady=20, command= lambda: minus_click()).grid(row=2, column=3)
button_multiply = Button(root, borderwidth=2, text="*", padx=37, pady=20, command= lambda: multiply_click()).grid(row=1, column=3)
